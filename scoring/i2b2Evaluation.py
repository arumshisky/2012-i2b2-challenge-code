...
if sys.version_info<(2,7):
    ...
    if __name__ == '__main__':
        ...
        parser.add_argument('-event', dest='entities_to_evaluate', action='store_const',\
                          const='event', default='all', help='select which entity(entities) to run evaluation on: event, timex, tlink, all(default)')
        parser.add_argument('-timex', dest='entities_to_evaluate', action='store_const',\
                          const='timex', default='all', help='select which entity(entities) to run evaluation on: event, timex, tlink, all(default)')
        parser.add_argument('-tlink', dest='entities_to_evaluate', action='store_const',\
                          const='tlink', default='all', help='select which entity(entities) to run evaluation on: event, timex, tlink, all(default)')
        parser.add_argument('-all', dest='entities_to_evaluate', action='store_const',\
                          const='all', default='all', help='select which entity(entities) to run evaluation on: event, timex, tlink, all(default)')
        ...
        if len(args.gold_file_des)==1 and len(args.system_file_des)==1:
                ...
                if len(goldFileList)==len(systemFileList):
                    ...
                    for file in goldFileList:
                        ...
                        if args.entities_to_evaluate in ['event', 'all']:
                            eventScores=eventEvaluation(goldDir+file,systemDir+file, args.span_match_type)
                            for i in range(2,11):
                                totaleventScores[i-2]+=eventScores[i]
                            goldDic=eventScores[0]
                            systemDic=eventScores[1]
                        if args.entities_to_evaluate in ['timex', 'all']:
                            timexScores=timexEvaluation(goldDir+file,systemDir+file,args.span_match_type,goldDic,systemDic)
                            for i in range(2,11):
                        ...
                        if args.entities_to_evaluate in ['tlink','all']:
                            if args.evaluation_option=='tempeval':
                                tlinkScores=evaluate_two_files(goldDir+file,systemDir+file,systemDic,goldDic)
                            elif args.evaluation_option=='oc':
                                tlinkScores=tlinkEvaluation(goldDir+file,systemDir+file,'OrigVsClosure',goldDic,systemDic)
                            elif args.evaluation_option=='cc':
                                tlinkScores=tlinkEvaluation(goldDir+file,systemDir+file,'ClosureVsClosure',goldDic,systemDic)
                            elif args.evaluation_option=='oo':
                                tlinkScores=tlinkEvaluation(goldDir+file,systemDir+file,'OrigVsOrig',goldDic,systemDic)
                            for i in range(0,4):
            ...
            EVENT: 
               Precision : \t"""+'{:.2}'.format(final_event_prec)+"""
               Recall : \t"""+'{:.2}'.format(final_event_recall)+"""
               Average P&R : \t"""+'{:.2}'.format(averagePR)+"""
               F measure : \t"""+'{:.2}'.format(fScore)+"""
               --------------
               Type :\t\t"""+'{:.2}'.format(final_event_type)+"""
               Polarity :\t"""+'{:.2}'.format(final_event_pol)+"""
               Modality :\t"""+'{:.2}'.format(final_event_mod)
            ...
            TIMEX3:
                Precision : \t"""+'{:.2}'.format(final_timex_prec)+"""
                Recall : \t"""+'{:.2}'.format(final_timex_recall)+"""
                Average P&R : \t"""+'{:.2}'.format(averagePR)+"""
                F measure : \t"""+'{:.2}'.format(fScore)+"""
                --------------
                Type :\t\t"""+'{:.2}'.format(final_timex_type)+"""
                Val :\t\t"""+'{:.2}'.format(final_timex_val)+"""
                Modifier :\t"""+'{:.2}'.format(final_timex_mod)
                        if args.entities_to_evaluate=='timex':
                            print '\n{:*^47}'.format('')
                    if args.entities_to_evaluate in ['tlink', 'all']:
            ...
            TLINK:
                Precision : \t"""+'{:.2}'.format(final_tlink_prec)+"""
                Recall : \t"""+'{:.2}'.format(final_tlink_recall)+"""
                Average P&R : \t"""+'{:.2}'.format(averagePR)+"""
                F measure : \t"""+'{:.2}'.format(fScore)+"""
                """+'\n{:*^47}'.format('')
                        if args.entities_to_evaluate=='tlink':
                            print "WARNING: when using tlink evaluation alone, please make sure that the Event/Timex ids in the gold standard matches the ids in the system output. Otherwise, the result will be wrong. If in double, use -all option."
                else:
                    print 'Error: gold directory and system directory don\'t match!'
